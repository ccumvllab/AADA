...
class Attention(nn.Module):
    ...
    def forward_cross(self, x, y, H, W):
        B, N, C = x.shape
        q = self.q(x).reshape(B, N, self.num_heads,
                              C // self.num_heads).permute(0, 2, 1,
                                                           3).contiguous()
...
class Block(nn.Module):
    ...
    def forward_cross(self, x, y, H, W):
        x = x + self.drop_path(self.attn.forward_cross(self.norm1(x), self.norm1(y), H, W))
        x = x + self.drop_path(self.mlp(self.norm2(x), H, W))
...
class MixVisionTransformer(BaseModule):
    ...
    def forward_cross_1(self, x, y):
        ...
        for i, blk in enumerate(self.block1):
            x = blk.forward_cross(x, y, H, W)
        x = self.norm1(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()

        return x
    
    def forward_cross_2(self, x, y):
        B = x.shape[0]
