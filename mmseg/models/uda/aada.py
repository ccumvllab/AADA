...
class AADA(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        # 是否開啟前期多使用random color jitter
        self.enable_CjAug = cfg['enable_CjAug']
        # random color jitter的使用機率
        self.CjAug_prob = 1.
        # lambda_adv
        self.adv_coef = cfg['adv_coef']
        # 是否要用對抗式訓練
        self.enable_adv = cfg['enable_adv']
        self.augmentation_init = cfg['augmentation_init']
    ...
    def augment(self, img, gt, means, stds, update=False, pw=None, only_caug=False):
        ...
        if update:
            aug_x, aug_x_t, swd, g_prob, c_prob, pw, p_stratgy, m_stratgy = self.trainable_aug(img, gt, feature, means, stds, pw, update)
        else:
            with torch.no_grad():
                aug_x, aug_x_t, swd, g_prob, c_prob, pw, p_stratgy, m_stratgy = self.trainable_aug(img, gt, feature, means, stds, pw, update, only_caug)
        if pw is not None:
            pw = pw.squeeze(dim=1)
    ...
    def forward_train(self,
        ...
        # 紀錄本次iteration擴增的使用情況，有用到為1，沒用到為0，設定上不使用為-1
        s_g_prob, s_c_prob = [None] * batch_size, [None] * batch_size
        if use_CJ:
            _, _, aug_img = strong_transform(strong_parameters, data=img.clone())
            aug_gt_semantic_seg = gt_semantic_seg
            self.train_log["Source aug prob"].append([s_g_prob, s_c_prob])
        else:
            aug_img, aug_gt_semantic_seg, _, s_g_prob, s_c_prob, _, \
                S_p_stratgy, S_m_stratgy = self.augment(img, gt_semantic_seg, means, stds)
            if self.enable_t_aug:
                self.train_log["Source aug prob"].append(S_p_stratgy)
                self.train_log["Source aug mang"].append(S_m_stratgy)
            else:
                self.train_log["Source aug prob"].append([s_g_prob, s_c_prob])
        ...
        self.train_log['Source loss'].append(clean_loss.item())
        ...
        if not self.source_only:
            ...
            t_g_prob, t_c_prob = [None] * batch_size, [None] * batch_size
            if use_CJ:
                strong_parameters = {
                    'mix': None,
                    'color_jitter': random.uniform(0, 1),
                ...
                aug_mixed_lbl = mixed_lbl
                self.train_log["Target aug prob"].append([t_g_prob, t_c_prob])
            else:
                aug_mixed_img, aug_mixed_lbl, _, t_g_prob, t_c_prob, mixed_seg_weight, \
                    T_p_stratgy, T_m_stratgy = self.augment(mixed_img, mixed_lbl, means, stds,pw=mixed_seg_weight)
                if self.enable_t_aug:
                    self.train_log["Target aug prob"].append(T_p_stratgy)
                    self.train_log["Target aug mang"].append(T_m_stratgy)
                else:
                    self.train_log["Target aug prob"].append([t_g_prob, t_c_prob])
            ...
            mix_loss.backward()
            self.train_log["Target loss"].append(mix_loss.item())
        # Masked Training
        if self.enable_masking and self.mask_mode.startswith('separate'):
            ...
            self.train_log["Mask loss"].append(masked_loss.item())
        else:
            self.train_log["Mask loss"].append(0)
        ...
        # print augmented img and segment pred.
        if self.local_iter % self.debug_img_interval == 0 and \
            ...
            for j in range(batch_size):
                ...
                # aug img
                subplotimg(axs[0][1], vis_aug_source_img[j], 
                           f'Aug Source g:{s_g_prob[j] if s_g_prob[j] is not None else -1}, c:{s_c_prob[j] if s_c_prob[j] is not None else -1}')
                subplotimg(axs[1][1], vis_aug_trg_img[j], 
                           f'Aug Target g:{t_g_prob[j] if t_g_prob[j] is not None else -1}, c:{t_c_prob[j] if t_c_prob[j] is not None else -1}')
                # GT
                subplotimg(
                    axs[0][2],
        ...
        if self.local_iter % self.update_augmodel_iter == 0:
            ...
            # augmentation
            aug_img, aug_gt_semantic_seg, s_swd, _, _, _, \
                S_p_stratgy, S_m_stratgy = self.augment(img, gt_semantic_seg, means, stds, update=True)
            # pred aug source image
            # 輸出student model的pred.
            ...
            # augmentation
            aug_mixed_img, aug_mixed_lbl, t_swd, _, _, _, \
                T_p_stratgy, T_m_stratgy = self.augment(mixed_img, mixed_lbl, means, stds, update=True)
            ...
            if self.local_iter>1500:
                ...
                for j in range(batch_size):
                    ...
                    # img
                    subplotimg(axs[0][0], vis_source_img[j], 'Source Image')
                    subplotimg(axs[0][1], vis_aug_source_img[j], f"Aug_Source")
                    subplotimg(
                        axs[1][0],
                        aug_gt_semantic_seg[j],
                        'Aug_Source GT',
                    ...
                    subplotimg(
                        axs[1][1],
                        vis_pred_student[j],
                        f'Student Pred. nscLoss: {loss_stu.item():04f}',
                        cmap='cityscapes')
                    subplotimg(
                        axs[1][2],
                        vis_pred_tea[j],
                        f'Teacher Pred. ceLoss: {loss_tea.item():04f}',
                        cmap='cityscapes')
                    for ax in axs.flat:
                            ax.axis('off')
                    plt.savefig(
                        os.path.join(Teacher_out_dir,
                                    f'{(self.local_iter + 1):06d}_{j}_{loss_tea.item():04f}.png'))
                    plt.close()
            ...
            if self.enable_wandb:
                ...
                if self.enable_adv:
                    ...
                    self.train_log["Student loss"].append(loss_stu.item())
                    self.train_log["Teacher loss"].append(loss_tea.item())
                    self.train_log["Source SWD loss"].append(s_swd.item())
                    self.train_log["Adv loss"].append(loss_adv.item())
                if self.enable_cross_attention:
                    self.wandb_run.log({
                                'iter': self.local_iter,
                    ...
                    self.train_log["Cross-attention loss"].append(cross_dis.item())
                    self.train_log["Target SWD loss"].append(t_swd.item())
                if self.enable_cross_attention and self.enable_adv:
                    self.train_log["Aug loss"].append(loss_adv.item()+loss_cross.item())
        self.local_iter += 1
        if self.local_iter==1 or self.local_iter>=(self.max_iters-1):
