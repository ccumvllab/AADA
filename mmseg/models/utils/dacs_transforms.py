...
def strong_transform(param, data=None, target=None):
    ...
    aug_data, target = color_jitter(
        ...
        data=data,
        target=target)
    aug_data, target = gaussian_blur(blur=param['blur'], data=aug_data, target=target)
    return data, target, aug_data
...
def renorm(img, mean, std):
    return (img*255.0).sub(mean).div(std)
